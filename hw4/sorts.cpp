...
//  hw4
//
//  Created by Alex Chen on 3/3/20.
...
{
    // FINISHED: You implement this.  Using the same criteria as compareStudent,
...
{
    // FINISHED: Using the insertion sort algorithm (pp. 311-313), sort s
...
int main()
{
      ...
      // destruction time.
    {
     vector<Student> auxStudents(students);
      ...
      // FINISHED:  Create a vector of Student pointers, and set each pointer
      //        to point to the corresponding Student in auxStudents.
        
     vector<Student*> studentPointers;
        
     for (vector<Student>::iterator itr = auxStudents.begin(); itr != auxStudents.end(); itr++)
         studentPointers.push_back(&*itr);

      // FINISHED:  Sort the vector of pointers using the STL sort algorithm
      //        with compareStudentPtr as the ordering relationship.
        
     sort(studentPointers.begin(), studentPointers.end(), compareStudentPtr);

      // FINISHED:  Using the now-sorted vector of pointers, replace each Student
